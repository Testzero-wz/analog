...
class ColorOutput:
    ...
    def print_info_green(self, message, **kwargs):
        # self.new_line(Fore.GREEN + '[*] {0}'.format(message) + Style.RESET_ALL)
        symbol = kwargs.get('symbol') if 'symbol' in kwargs.keys() else "*"
        if self.system == "Windows":
            self.new_line(Fore.GREEN + Style.NORMAL + "[" + symbol + "] {0}".format(message) + Style.RESET_ALL,
                          **kwargs)
        else:
            self.new_line(Fore.GREEN + Style.NORMAL + "[" + symbol + "] {0}".format(message) + Style.RESET_ALL,
                          **kwargs)
    ...
    def print_value(self, variable, value, symbol="*"):
        self.new_line(Fore.LIGHTYELLOW_EX + Style.NORMAL + '[' + symbol + '] {0} => {1}'.format(
            Fore.LIGHTBLUE_EX + variable + Fore.LIGHTYELLOW_EX,
            Fore.LIGHTBLUE_EX + str(value)) + Style.RESET_ALL)

    def print_plain_value(self, variable, value, symbol="*"):
        self.new_line(Fore.LIGHTYELLOW_EX + Style.NORMAL + '[' + symbol + '] {0} => {1}'.format(
            Style.RESET_ALL + variable + Fore.LIGHTYELLOW_EX,
            Style.RESET_ALL + str(value)) + Style.RESET_ALL)
